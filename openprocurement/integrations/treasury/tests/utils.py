...
class TestUtils(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.keys = [
            'fundingKind', 'NBUdiscountRate', 'yearlyPaymentsPercentageRange', 'noticePublicationDate',
    ...
    def test_handle_common_tenders(self, *mocks):
        ...
        mocks[0].info.assert_called_with(
            'Handle common tender {}'.format(self.tender['id']), extra={'MESSAGE_ID': 'handle_common_tenders'}
        )

    @patch('openprocurement.integrations.treasury.databridge.utils.generate_milestones')
    def test_handle_esco_tenders(self, mocked_generete_milestones, *mocks):
        ...
        mocks[0].info.assert_called_with(
            'Handle esco tender {}'.format(self.tender['id']), extra={'MESSAGE_ID': 'handle_esco_tenders'}
        )
    ...
    def test_handle_esco_tenders_multilot(self, mocked_generate_m, *mocks):
        ...
        mocks[0].debug.assert_has_calls(
            [call('Fill contract {} values from lot {}'.format(contract['id'], test_lot['id']))]
        )
        ...
        mocks[0].warn.assert_has_calls(
            [call('Not found related award for contract {} of tender {}'.format(contract['id'], tender['id']))]
        )
        ...
        mocks[0].critical.assert_has_calls(
            [call('Not found related lot for contract {} of tender {}'.format(contract['id'], tender['id']),
    ...
    def test_fill_base_contract_data(self, *mocks):
        ...
        self.assertEquals(
            all(key in contract.keys() for key in ['tender_id', 'procuringEntity']), True
        )

        #  testing deliveryDate mistmach
        contract = deepcopy(self.contract)
        contract['items'][0]['deliveryDate'] = dict()
        contract['items'][0]['deliveryDate']['startDate'] = (datetime.now()+timedelta(days=1)).isoformat()
        contract['items'][0]['deliveryDate']['endDate'] = datetime.now().isoformat()
        fill_base_contract_data(contract, self.tender)
        info_calls.append(
            call('startDate value cleaned.', extra={
        ...
        tender['lots'] = [lot]
        award = [aw for aw in tender['awards'] if aw['id'] == contract['awardID']][0]
        fill_base_contract_data(contract, tender)

        mocks[0].debug.assert_has_calls(
            [call('Copying items matching related lot {}'.format(award['lotID']))]
        )
        ...
        award['items'] = [item]
        fill_base_contract_data(contract, tender)
        mocks[0].debug.has_calls(
            [call('Copying items from related award {}'.format(award['id']))]
        )
        self.assertEquals(contract['items'], award['items'])  # copy check

    def test_journal_context(self, *mocks):
        self.assertEquals(
            journal_context(record={}, params={'test': 'test'}), {'JOURNAL_test': 'test'}
        )
    ...
    def test_generate_milestones(self, *mocks):
        ...
        ) if 'period' in contract and 'startDate' in contract['period'] else parse_date(contract['dateSigned'])
        ...
        for seq_number, milestone in enumerate(milestones):
            ...
            elif contract_end_date.year >= parse_date(milestone['period']['startDate']).year and seq_number != 1:
        ...
        contract_delta_15_years = contract_delta_15_years.replace(year=contract_delta_15_years.year+15)
        self.assertEquals(
